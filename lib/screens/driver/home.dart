...
class _MyWidgetState extends State<Home> {
  ...
  Future<List<Tricycle>> getTricycle() async {
    ...
    final response = await http.post(Uri.parse(tricycleShow), headers: {
      'Accept': 'application/json',
      'Authorization': 'Bearer $token'
    }, body: {
  ...
  Future<List<Booking>> DriverBookingList() async {
    final response = await http.post(Uri.parse(driverBookingList), body: {
  ...
  void getUser() async {
    ...
    if (response.error == null) {
      setState(() {
        ...
        if (userimage == '') {
          image = imageBaseUrl +
              'images/yBTmndaYLCcHHkpzqqdSqaoj2RJHK73PzkwVjYTV.png';
        } else {
  ...
  void ActiveClick() async {
    ...
    ApiResponse response = await active_driver(
        user_id.toString(), currentTerminal_id.toString(), Tryk_id, '1');
    if (response.error == null) {
      ScaffoldMessenger.of(context)
  ...
  void InactiveClick() async {
    int user_id = await getUserId();
    ApiResponse response = await active_driver(
        user_id.toString(), currentTerminal_id.toString(), Tryk_id, '0');
    if (response.error == null) {
      ScaffoldMessenger.of(context)
      ...
      (active == 1
          ? Container(
              decoration: new BoxDecoration(
                boxShadow: [
                  BoxShadow(
                    color: Color.fromARGB(255, 151, 170, 159).withOpacity(.5),
                        ...
                        children: [
                          Row(
                            children: [
                              ...
                              Text(passenger_location,
                                  style: TextStyle(
                                      fontSize: 13,
                                      color: Color.fromARGB(255, 6, 149, 52)))
                            ],
                          ),
                          SizedBox(
                            height: 20,
                                                                ...
                                                                Container(
                                                                  child: Column(
                                                                    children: [
                                                                      Text(
                                                                        'Plate Number: ' +
                                                                            tricycles.plate_number.toString(),
                                                                        textAlign:
                                                                            TextAlign.left,
                                                                      ),
                                                                      Text(
                                                                        'Body Number: ' +
                                                                            tricycles.body_number.toString(),
                                                                        textAlign:
                                                                            TextAlign.left,
                                                                      ),
                                                                      Text(
                                                                        'max Passenger: ' +
                                                                            tricycles.max_passenger.toString(),
                                                                        textAlign:
                                                                            TextAlign.left,
                                                                      ),
                                                                    ],
